...
player1 = Person(name="PhoenixHeat", hp=5234, mp=150, atk=400, df=34, magic=player_spells, items=player_items)
player2 = Person(name="NightRider ", hp=3440, mp=350, atk=333, df=34, magic=player_spells, items=healer_items)
player3 = Person(name="BoomBlaster", hp=6460, mp=100, atk=555, df=34, magic=player_spells, items=player_items)

enemy1 = Person(name="EvilDevil   ", hp=1300, mp=300, atk=530, df=325, magic=enemy_spells, items=[])
enemy2 = Person(name="ChillKill   ", hp=15200, mp=250, atk=650, df=25, magic=enemy_spells, items=[])
enemy3 = Person(name="BlitzAlpha  ", hp=1200, mp=300, atk=430, df=325, magic=enemy_spells, items=[])
...
while running:
    ...
    for player in players:
        player.get_stats()
        if player.hp == 0:
            print(player.name.replace(" ","") + " has died!")
        elif player.hp > 0:  # Check which player is alive and append to alive players
    ...
    for enemy in enemies:
        ...
        elif enemy.hp > 0:  # Check which enemy is alive and append to alive enemies
    ...
    if len(alive_players) == 0:
        print(bcolors.OKGREEN + "Enemies have defeated you!" + bcolors.ENDC)
        break
